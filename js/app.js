...
function renderFatalError(err){
  ...
  root.innerHTML = `
    <section class="shell">
      <div class="empty">
        <h2>تعذر تشغيل الواجهة</h2>
        <p>لم يتم تحميل بيانات الباقات. تأكد من وجود ملفات البيانات داخل مجلد <code>data</code>.</p>
        <small>${escapeHtml(err?.message || 'خطأ غير معروف')}</small>
      </div>
    </section>
  `;
}
